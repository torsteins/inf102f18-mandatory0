...
#### *Clarifications*
  * *In task (b), you may use whichever implementation of the union-find data structure you like. Quick-find and quick-union are two examples of possible implementations; however, neither is likely to be fast enough for this exercise without further optimization.*
 ...
 * *Even though `add(e)` and `add(0, e)` should both take O(1) time, that does not mean that `add(k, e)` should take O(1) time - an implementation which does it in O(k) time is fine. The same holds for `remove(k)`.*
